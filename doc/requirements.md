...
- **インポート**
  ...
  - **外部音源ファイルのインポート**
    ...
    - WebからURLを指定してWAV音源を取り込み
    - 新しいドラム音源として使用可能
    - キーマッピングに追加してリアルタイム演奏
    - カスタム音源によるオリジナルビート作成

- **PWA対応**
  - デスクトップ・モバイルへのアプリインストール
...
### 5.4 PWA対応
- デスクトップ・モバイルへのインストール可能
- オフライン環境でも基本機能が動作
- アプリライクな操作感（スタンドアロン表示）
- ブラウザインストールプロンプトの表示
...
- **manifest.json**
  ...
  - 表示モード: standalone（アプリライク表示）
  - アイコン: 192x192, 512x512（ユーザー提供）

- **Service Worker**
  - オフラインキャッシュ戦略
  - 音源ファイル（/public/sounds/）のキャッシュ
  - 静的アセットのキャッシュ
...
- ✅ タイムラインが視覚的に分かりやすい
- ✅ JSON/CSVエクスポートが正常動作
- ✅ JSON/CSVインポートで録音データを復元できる
- ✅ WAVファイルインポートでカスタム音源を使用できる
- PWAとしてインストール可能
- オフラインで基本機能が動作
...
- キーボード操作前提（モバイル非対応）
- 簡易的なタイミング記録（波形編集不可）
- インポートした音源ファイルはブラウザセッション内でのみ有効（ページリロード時に再アップロード必要）
